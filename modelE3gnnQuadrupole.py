...
class E3nnModelQ(torch.nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(irreps_out.dim, 9) # to match the 3D quadrupole moment
...
for data in dataset:
    data.quadrupole = data.quadrupole.view(1, 9)
...
def evaluate(model, data_loader):
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            target = data.quadrupole.view(-1, 9)  # Reshape target
            error = prediction - target  # Use reshaped data.quadrupole as target
            total_abs_error += torch.abs(error).sum().item()
            total_sq_error += torch.pow(error, 2).sum().item()
            total_variance += torch.var(target, unbiased=False).item() * (target.shape[0] - 1)  # Use reshaped data.quadrupole
...
for epoch in range(n_epochs):
    ...
    for batch in train_loader:
        ...
        optimizer.zero_grad()
        out = model(batch)
        target = batch.quadrupole.view(-1, 9)
