...
from EGNN_PointNet import EquivariantGNN_PointNet
...
# Define models
dipole_model = EquivariantGNN_PointNet(num_node_features, hidden_dim, output_dim_dipoles, num_node_types).to(device)  # 3 for dipole
quadrupole_model = EquivariantGNN_PointNet(num_node_features, hidden_dim, output_dim_quadrupoles, num_node_types).to(device)  # 6 for quadrupole
...
for epoch in range(100):
    ...
    if valid_loss < best_valid_loss:
        ...
        epochs_no_improve = 0
        # Save the model
        torch.save({'dipole_model_state_dict_PointNet': dipole_model.state_dict(),
                    'quadrupole_model_state_dict_PointNet': quadrupole_model.state_dict()}, 'best_model_PointNet.pth')
    else:
        epochs_no_improve += 1
...
# Load the best model
checkpoint = torch.load('best_model_PointNet.pth')
dipole_model.load_state_dict(checkpoint['dipole_model_state_dict_PointNet'])
quadrupole_model.load_state_dict(checkpoint['quadrupole_model_state_dict_PointNet'])
...
print(f'Dipole Model PointNet: MAE = {mae_dipole}, R^2 = {r2_dipole}')
print(f'Quadrupole Model PointNet: MAE = {mae_quadrupole}, R^2 = {r2_quadrupole}')