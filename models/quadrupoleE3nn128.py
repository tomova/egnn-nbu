...
max_num_nodes = 0
dataset = []
for atom_features, atom_positions, _, bond_features, _, quadrupole in data:
...
for epoch in range(1000):
    ...
    with torch.no_grad():
        ...
        if avg_val_loss_l1 < best_val_loss:
            ...
            torch.save(net.state_dict(), 'best_model_e3nn_quadrupole_mae128.pth')


# Testing
net.load_state_dict(torch.load('best_model_e3nn_quadrupole_mae128.pth'))
net.eval()
with torch.no_grad():
    test_loss_l1 = 0
    test_loss_mse = 0
...
# to get the predictions for the quadrupole moment.        