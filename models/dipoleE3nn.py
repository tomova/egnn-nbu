...
class DipolePredictorE3NN(nn.Module):
    def __init__(self, max_num_nodes):
        ...
        irreps_hidden = o3.Irreps("5x0e + 5x1e") 
        irreps_out = o3.Irreps("1x1e") 

        # Equivariant convolution layer
        self.conv = Convolution(irreps_in, irreps_hidden, max_radius=1.0)
        
        # Fully connected layers
        layers = [nn.Linear(irreps_hidden.dim, irreps_hidden.dim), nn.SiLU(),
                  nn.Linear(irreps_hidden.dim, irreps_out.dim)]
        self.fully_connected = FullyConnectedNet([irreps_hidden.dim] * 2, layers)

    def forward(self, feats, coors, adj_mat):
        # Equivariant convolution
        feats_out = self.conv(feats, coors, coors)

        # Mean pooling across nodes, preserving batch and feature dims
        graph_embedding = feats_out.mean(dim=1)

        # Pass through fully connected layers
        output = self.fully_connected(graph_embedding)
